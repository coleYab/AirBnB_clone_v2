...
def display_city_by_state():
    """
    displays city by state in the code given
    """
    states_list = storage.all('State').values()
    cities_list = storage.all('Cities').values()
    state_cities = {}
    for state in set(states_list):
        i = f'{state.id}-{state.name}'
        state_cities[i] = []
        for cities in [city for city in cities_list if city.id == state.id]:
            state_cities[i].append(f"{cities.id}-{cities.name}")
    return render_template('8-cities_by_states.html',
                           state_cities_list=states_list)
